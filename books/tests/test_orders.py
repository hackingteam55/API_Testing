...
class TestOrders:
    ...
    def test_add_valid_order(self):
        r = add_order(self.token, 5, 'Shobo')
    ...
    def test_get_order(self):
        book_id = add_order(self.token, 1, 'Thor').json()['orderId']
        r = get_order(self.token, book_id)
        assert r.status_code == 200, 'code not ok'
        assert r.json()['id'] == book_id, 'id not ok'
        assert r.json()['bookId'] == 1, 'book id not ok'
        assert r.json()['customerName'] == 'Thor', 'customerName not ok'
        assert r.json()['quantity'] == 1, 'quantity not ok'
        # cleanup
        delete_order(self.token, book_id)
    ...
    def test_patch_invalid_order_id(self):
        r = edit_order(self.token, '123abc', 'Thor2')
        assert r.status_code == 404, 'code not ok'
        assert r.json()['error'] == 'No order with id 123abc.', 'error no ok'
